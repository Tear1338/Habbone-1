...
import { MessageSquare, FileText, MessagesSquare, Coins } from 'lucide-react'
...
function PodiumAvatar({ nick, rank, score }: { nick: string; rank: number; score: number }) {
  const heights = [160, 120, 100] // 1st, 2nd, 3rd podium heights
    ...
    <div className="flex flex-col items-center" style={{ order: rank === 0 ? 1 : rank === 1 ? 0 : 2 }}>
      ...
      <div className="relative mb-1">
        ...
        <img
          src={avatarUrl(nick)}
          alt={nick}
          className="h-[64px] w-auto image-pixelated drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
          loading="lazy"
        />
      </div>

      {/* Nick */}
      <span className="mb-1 max-w-[100px] truncate text-[12px] font-bold text-white">
        {nick}
      </span>

      {/* Score */}
      <span className="mb-2 text-[11px] text-[#BEBECE]">
      ...
      {entries.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
          ...
          <div className="rounded-[8px] border border-[#1F1F3E] bg-[#272746] p-4">
            ...
            {rest.length === 0 ? (
              <div className="py-6 text-center text-[13px] text-[#BEBECE]/50">
              ...
              <div className="space-y-1.5">
                {rest.map((entry, i) => (
                  <div
                    key={entry.nick}
                    className="flex items-center gap-3 rounded-[4px] border border-[#1F1F3E] bg-[#1F1F3E] px-3 py-2 transition hover:border-white/10 hover:bg-[#303060]"
                  >
                    <span className="flex h-[28px] w-[28px] shrink-0 items-center justify-center rounded-full bg-[rgba(255,255,255,0.08)] text-[12px] font-bold text-[#BEBECE]">
                      {i + 4}
                    </span>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={buildHabboAvatarUrl(entry.nick, { direction: 2, head_direction: 3, headonly: 1, size: 's', img_format: 'png' })}
                      alt={entry.nick}
                      className="h-[28px] w-auto image-pixelated"
                      loading="lazy"
                    />
                    <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-[#DDD]">
                      {entry.nick}
                    </span>
                    <span className="shrink-0 text-[13px] font-bold text-[#2596FF]">
